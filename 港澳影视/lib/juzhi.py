...
class Spider(BaseSpider):
    ...
    def _init_host(self):
        ...
        if not self.host:
            for fallback_host in self.FALLBACK_HOSTS:
                try:
                    test_url = fallback_host + "/api/v3/drama/getCategory?orderBy=type_id"
                    if self.session:
    ...
    def _init_dynamic_key(self):
        """初始化动态密钥，失败时使用公钥作为fallback"""
        if self.host:
            try:
                self._load_dynamic_key()
            except Exception:
                self.dynamic_key = self.public_key
    ...
    def _check_crypto():
        if AES is None or RSA is None:
            raise RuntimeError("缺少 pycryptodome：请确保 Python 环境可导入 Crypto")
    ...
    def _ensure_host(self):
        ...
        if not self.host or not self.session:
            ...
            self._init_dynamic_key()
        if not self.host:
            raise RuntimeError("橘汁接口域名获取失败，请检查网络或配置")

    def _request_with_retry(self, method, path, body=None, is_protobuf=True, max_retries=2):
        """带重试的请求方法"""
        last_error = None
            ...
            try:
                ...
                if method == "post":
                    r = self.session.post(
                        ...
                        headers=self._public_headers(is_protobuf), timeout=18
                    )
                else:
                    r = self.session.get(
                        self.host + path,
    ...
    def _post(self, path, body):
        try:
            r = self._request_with_retry("post", path, body=body, is_protobuf=True)
            return r.content
        except Exception as e:
            raise RuntimeError(f"橘汁请求失败: {e}")

    def _get_json(self, path):
        try:
            r = self._request_with_retry("get", path, is_protobuf=False)
            return r.json()
        except Exception as e:
            raise RuntimeError(f"橘汁请求失败: {e}")
    ...
    def homeVideoContent(self):
        try:
            ...
            data = obj.get("data", "")
            if str(self.cfg.get("decrypt", "1")) != "0" and data:
                data = self._aes_decrypt_ecb(data, self.data_key)
                data = self._aes_decrypt_ecb(data, self.data_iv)
            ...
            for block in arr or []:
                for section in block.get("sections") or []:
                    for vod in section.get("vodList") or []:
                        pic = (vod.get("coverImage") or {}).get("path", "")
                        videos.append({"vod_id": str(vod.get("id", "")), "vod_name": vod.get("name", ""), "vod_pic": pic, "vod_remarks": vod.get("remark", "")})
            return {"list": videos}
        except Exception:
            return {"list": []}
