# -*- coding: utf-8 -*-
# by @嗷呜
import re, os, sys, time, json, uuid, binascii
...
class Spider(Spider):
    ...
    def getohost(self):
        url = 'https://bianyuan1.oss-cn-hangzhou.aliyuncs.com/bianyuan20250605.json'
        response = self.fetch(url, headers=self.headers).text
        try:
            data = json.loads(response)
        except (json.JSONDecodeError, TypeError):
            data_ = self.daes(response, b64decode('kzCsPVrVY4+AGjtOFW8Wug=='), 1)
    ...
    def daes(self, encrypted_data, key, hexi=0):
        if hexi == 0:
            key = key.encode('utf-8')
